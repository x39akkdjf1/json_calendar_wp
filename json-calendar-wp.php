...
final class JSON_Calendar_WP {
	...
	private function machine_time_value( $time ) { $value = trim( (string) $time ); if ( preg_match( '/^(\d{2}:\d{2}(?::\d{2})?(?:\.\d+)?(?:Z|[+\-]\d{2}:\d{2})?)$/', $value, $matches ) ) return $matches[1]; try { $datetime = new DateTimeImmutable( $value ); return $datetime->format( 'H:i:s' ); } catch ( Exception $exception ) { return ''; } }
