...
final class JSON_Calendar_WP {
	...
	public function render_shortcode( $atts ) {
		...
		$output = $this->get_shortcode_styles( $id ) . '<div id="' . $id . '" class="json-calendar' . ( $is_next ? ' json-calendar-next' : ( $is_archive ? ' json-calendar-archive' : '' ) ) . '"><ul class="json-calendar-list">';

		foreach ( $entries as $entry ) {
			$title = $this->value( $entry, array( 'title', 'name', 'summary' ), __( 'Untitled event', 'json-calendar-wp' ) );
			$image = $this->first_image( $entry );
			$event_reference = $this->get_event_reference( $entry );
			$event_url = isset( $event_urls[ $event_reference ] ) ? $event_urls[ $event_reference ] : '';
	...
	private function upsert_event_post( $entry, $url, $reference, &$existing_posts ) {
		...
		if ( $description ) {
			$post_content .= '<div class="json-calendar-event-description" style="margin-top:1.5rem;">' . wp_kses_post( $description ) . '</div>';
		}
