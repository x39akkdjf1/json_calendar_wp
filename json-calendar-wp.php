...
final class JSON_Calendar_WP {
	...
	private function machine_time_value( $time ) { $value = trim( (string) $time ); if ( preg_match( '/(?:^|T|\s)(\d{2}:\d{2}(?::\d{2})?(?:\.\d+)?)(?:\s*(Z|[+\-]\d{2}:\d{2}))?\s*$/', $value, $matches ) ) return $matches[1] . ( ! empty( $matches[2] ) ? $matches[2] : '' ); if ( preg_match( '/(?:Z|[+\-]\d{2}:\d{2})\s*$/', $value ) ) return ''; try { $datetime = new DateTimeImmutable( $value ); return $datetime->format( 'H:i:s' ); } catch ( Exception $exception ) { return ''; } }
