		...
		foreach ( $entries as $entry ) {
			...
			if ( $is_next ) {
				...
				if ( $image && $event_url ) {
					...
					$output .= '<a class="json-calendar-card-link" href="' . esc_url( $event_url ) . '" aria-label="' . esc_attr( $card_label ) . '" title="' . esc_attr( $card_label ) . '"><img class="json-calendar-image" src="' . esc_url( $image ) . '" alt="' . esc_attr( $title ) . '" loading="lazy" /><span class="json-calendar-card-overlay" aria-hidden="true">' . esc_html__( 'Mehr', 'json-calendar-wp' ) . '</span></a>';
				} else {
					if ( $image ) $output .= '<img class="json-calendar-image" src="' . esc_url( $image ) . '" alt="' . esc_attr( $title ) . '" loading="lazy" />';
