...
final class JSON_Calendar_WP {
	...
	public function render_shortcode( $atts ) {
		...
		if ( false === $data ) {
			$response = wp_safe_remote_get( $url, array( 'timeout' => 10, 'headers' => array( 'Accept' => 'application/json' ), 'user-agent' => 'JSON Calendar WordPress Plugin/1.6.2' ) );
		...
		foreach ( $entries as $entry ) {
			...
			if ( $is_next ) {
				...
				$output .= '<div class="json-calendar-details"><div class="wp-block-cover__inner-container has-global-padding is-layout-constrained wp-block-cover-is-layout-constrained"><span class="json-calendar-next-heading">' . esc_html( get_option( self::OPTION_NEXT_HEADING, __( 'Next up', 'json-calendar-wp' ) ) ) . '</span><h1 class="json-calendar-title">' . esc_html( $title ) . '</h1><span class="json-calendar-more">' . esc_html__( 'mehr', 'json-calendar-wp' ) . '</span></div></div>';
			} else {
				if ( $image ) $output .= '<img class="json-calendar-image" src="' . esc_url( $image ) . '" alt="' . esc_attr( $title ) . '" loading="lazy" />';
